...
// ─── Exportar Excel ───────────────────────────────────────────────────────────
function buildExcel(cuidadores: Cuidador[], allAsistencias: AsistenciaCuidador[]): XLSX.WorkBook {
  const wb    = XLSX.utils.book_new();
  const fechas = [...new Set(allAsistencias.map(a => a.fecha))].sort();
  const asistMap = new Map<string, boolean>();
  allAsistencias.forEach(a => asistMap.set(`${a.cuidador_id}_${a.fecha}`, a.asistio));

  // Hoja 1: pivot niño × día
  const pivotRows = cuidadores.map(n => {
  ...
  XLSX.utils.book_append_sheet(wb, wspivot, 'AsistenciaCuidador por Día');

  // Hoja 2: resumen faltantes
  const totalDias = fechas.length;
  const summaryRows = cuidadores.map(n => {
    const registros = allAsistencias.filter(a => a.cuidador_id === n.id);
    ...
    return {
      ...
      '% AsistenciaCuidador': `${pct}%`,
      'Acudiente': n.nombre ?? '', 'Teléfono': n.telefono ?? '',
    };
...
function NinoHistoryCard({ nino, onClose }: { nino: Cuidador; onClose: () => void }) {
  const [history, setHistory]   = useState<AsistenciaCuidador[]>([]);
    ...
    const load = async () => {
      ...
      try {
        if (IS_MOCK_MODE) {
          ...
          const mock = ([] as AsistenciaCuidador[]).filter(a => a.cuidador_id === nino.id);
          setHistory(mock);
        } else {
          const { data, error } = await supabase
            .from('asistencia')
            .select('*')
            .eq('cuidador_id', nino.id)
            .order('fecha');
          if (error) throw error;
          setHistory((data || []) as AsistenciaCuidador[]);
        }
      } catch (err) {
        console.error(err);
        setHistory([]);
      ...
      {history.length > 0 && (
        <div className="px-5 py-3 border-b border-slate-100">
          <div className="flex justify-between text-xs font-bold text-slate-500 mb-1.5">
            <span>AsistenciaCuidador total</span>
...
export default function ReportsCuidadores() {
  ...
  const [selectedNino, setSelectedNino] = useState<Cuidador | null>(null);
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      let allAsistencias: AsistenciaCuidador[] = [];
      if (IS_MOCK_MODE) {
        allAsistencias = Object.values([]);
      } else {
        const { data, error } = await supabase.from('asistencia').select('*').order('fecha');
        if (error) throw error;
        allAsistencias = (data || []) as AsistenciaCuidador[];
      }
      const ninosTarget = selectedGrupo === 'Todos'
        ? cuidadores
        : cuidadores.filter(n => (n.grupo ?? '').toUpperCase() === selectedGrupo);
  ...
  // ── Filtros ────────────────────────────────────────────────────────────────
  const ninosByGrupo = cuidadores.filter(n =>
  ...
  return (
    <div className="flex flex-col gap-5 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-6">
      ...
      {ninosByGrupo.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm px-4 py-3">
          <div className="flex justify-between text-xs font-bold text-slate-500 mb-2">
            <span>AsistenciaCuidador del día</span>
      ...
      <button
        ...
        disabled={exporting || cuidadores.length === 0}
                  ...
                  <div className="pt-3 border-t border-slate-100">
                    <div className="flex items-center justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <p className="text-xs text-slate-400 uppercase tracking-wider font-bold">Teléfono</p>
                        <p className="font-semibold text-slate-700 text-sm truncate">{nino.nombre || '—'}</p>
                        {nino.telefono && <p className="text-xs text-slate-400 mt-0.5">{nino.telefono}</p>}
                      </div>
                      <a
                        href={`tel:${nino.telefono || ''}`}
